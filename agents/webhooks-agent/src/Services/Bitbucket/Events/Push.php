 ...
 * Caution: BitBucket is not providing entirety of data in push report, only the last commit and information regarding previous HEAD
 * Therefore, this event will not report all commits in a push, just the most recent one; this appears to be intentional behaviour of BitBucket API
 */
class Push implements EventInterface
{
    ...
    private function prepareMessages($data)
    {
        ...
        foreach ($data->push->changes as $change) {
            array_push($messages, $this->getArrayFromCommit($change));
        }

        return $messages;
    }

    private function getArrayFromCommit($change)
    {
